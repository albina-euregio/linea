...
import { TouchZoom } from "../../linea-plot/touch-zoom";
...
export class OptsHelper {
  static UpdateAxisLabels(
    ...
    // Left Y-axis label
    const leftLabelOffset = u.ctx.measureText(leftLabel).width / 2 - 10;
    const leftLabelX = chartLeft + leftLabelOffset;

    u.ctx.save();
    u.ctx.textAlign = "center";
    u.ctx.fillStyle = leftFillStyle;
    u.ctx.fillText(leftLabel, leftLabelX, yPosition);
    u.ctx.restore();

    const leftLabelWidth = u.ctx.measureText(leftLabel).width;
    ...
    // Right Y-axis label
    const rightLabelOffset = u.ctx.measureText(rightLabel).width / 2 - 10;
    const rightLabelX = chartLeft + chartWidth - rightLabelOffset;

    let fontSize = u.ctx.font ? parseInt(u.ctx.font, 10) : 12;

    const rightLabelWidth = u.ctx.measureText(rightLabel).width;
    ...
    u.ctx.save();
    u.ctx.textAlign = "center";
    u.ctx.fillStyle = rightFillStyle;
    u.ctx.fillText(rightLabel, rightLabelX, rightLabelY);
    u.ctx.restore();
