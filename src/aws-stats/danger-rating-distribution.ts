...
export class DangerRatingChart extends AbstractChart {
    ...
    render(): void {
        ...
        if (counts.every((value) => value === 0)) {
            ...
            empty.textContent = "No danger rating data available";
        ...
        this.createPlot(opts_danger_rating_distribution, [xValues]);
        const sum = counts.reduce((a, b) => a + b, 0);

        for (let i = 0; i < dangerDistributionOrder.length; i++) {
            const sparseSeries = xValues.map((_x, index) => (index === i ? counts[i] / sum * 100 : null));
